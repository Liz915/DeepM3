#!/usr/bin/env python3
"""
E10: Timestamp Robustness - Gaussian noise + random dropout on timestamps.
Tests whether ODE degrades more gracefully than GRU under temporal corruption.
"""
import argparse
import copy
import os
import sys
import torch
import yaml
import numpy as np
import pandas as pd
from tqdm import tqdm

sys.path.append(os.getcwd())
from src.data.dataset import MovieLensDataset
from src.dynamics.modeling import DeepM3Model
from src.dynamics.gru_baseline import GRUBaseline


def pick_device(name="auto"):
    if name == "cuda" and torch.cuda.is_available():
        return torch.device("cuda")
    if name == "mps" and torch.backends.mps.is_available():
        return torch.device("mps")
    if name == "auto":
        if torch.cuda.is_available():
            return torch.device("cuda")
        if torch.backends.mps.is_available():
            return torch.device("mps")
    return torch.device("cpu")


def infer_hidden_dim(state_dict):
    w = state_dict.get("item_emb.weight")
    if w is not None and getattr(w, "ndim", 0) == 2:
        return int(w.shape[1])
    gru_w = state_dict.get("gru.weight_ih_l0")
    if gru_w is not None and getattr(gru_w, "ndim", 0) == 2:
        return int(gru_w.shape[0] // 3)
    return None


def compute_ndcg(scores, k=10):
    _, indices = torch.topk(scores, k, dim=-1)
    if 0 in indices[0]:
        rank = (indices[0] == 0).nonzero(as_tuple=True)[0].item()
        return 1.0 / np.log2(rank + 2)
    return 0.0


def corrupt_timestamps(t, mode="noise", noise_std=0.5, drop_rate=0.3):
    """Apply temporal corruption.
    mode='noise': add Gaussian noise
    mode='drop':  randomly zero-out timestamps
    mode='both':  both noise + drop
    """
    t_out = t.clone()
    if mode in ("noise", "both"):
        t_out = t_out + torch.randn_like(t_out) * noise_std
        t_out = t_out.clamp(min=0)
        # Re-sort to keep monotonicity
        t_out, _ = t_out.sort(dim=-1)
    if mode in ("drop", "both"):
        mask = torch.rand_like(t_out) > drop_rate
        t_out = t_out * mask.float()
    return t_out


def eval_model_robust(model, dataset, device, solver, corrupt_mode=None,
                       num_neg=100, seed=42, noise_std=0.5, drop_rate=0.3, topk=10):
    rng = np.random.default_rng(seed)
    ndcg_list = []
    model.eval()

    with torch.no_grad():
        desc = f"Eval {solver} ({corrupt_mode})"
        for idx in tqdm(range(len(dataset)), desc=desc, leave=False):
            sample = dataset[idx]
            x = sample["x"].unsqueeze(0).to(device)
            t = sample["t"].unsqueeze(0).to(device)
            dt = sample["dt"].unsqueeze(0).to(device) if "dt" in sample else None
            pos = int(sample["pos"].item())

            blocked = {pos}
            blocked.update(int(v) for v in sample["x"] if int(v) > 0)
            negs = []
            while len(negs) < num_neg:
                c = int(rng.integers(1, dataset.n_items))
                if c not in blocked:
                    negs.append(c)
            cand = torch.tensor([pos] + negs, dtype=torch.long, device=device)

            if corrupt_mode:
                t = corrupt_timestamps(t, mode=corrupt_mode,
                                       noise_std=noise_std, drop_rate=drop_rate)
                if dt is not None:
                    dt = torch.zeros_like(t)
                    dt[:, 1:] = t[:, 1:] - t[:, :-1]

            if solver == "baseline":
                user_emb = model(x)
            else:
                user_emb = model(x, t, dt=dt)

            cand_emb = model.get_item_embedding(cand)
            scores = torch.matmul(user_emb, cand_emb.t())
            ndcg_list.append(compute_ndcg(scores, k=topk))

    return np.mean(ndcg_list)


def main():
    parser = argparse.ArgumentParser(description="Timestamp robustness analysis")
    parser.add_argument("--config", default="configs/config.yaml")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--num_neg", type=int, default=100)
    parser.add_argument("--topk", type=int, default=10)
    parser.add_argument("--device", default="auto", choices=["auto", "cpu", "cuda", "mps"])
    parser.add_argument("--output_dir", default="results/ml1m")
    parser.add_argument("--dataset", type=str, default="ml1m", choices=["ml1m", "amazon"])
    parser.add_argument("--data_dir", type=str, default="")
    parser.add_argument("--hidden_dim", type=int, default=128)
    parser.add_argument("--baseline_ckpt", type=str, default="")
    parser.add_argument("--euler_ckpt", type=str, default="")
    parser.add_argument("--rk4_ckpt", type=str, default="")
    args = parser.parse_args()

    device = pick_device(args.device)
    with open(args.config, "r") as f:
        cfg = yaml.safe_load(f)

    data_dir = args.data_dir or ("data/amazon" if args.dataset == "amazon" else "data")
    dataset = MovieLensDataset(mode="test", data_dir=data_dir)
    print(f"Test samples: {len(dataset)} | device={device}")

    models = {}

    ds = args.dataset
    dim = args.hidden_dim

    def get_ckpt(tag):
        if ds == "ml1m":
            legacy = {
                "baseline": "checkpoints/model_gru_baseline.pth",
                "euler": "checkpoints/model_ode_euler.pth",
                "rk4": "checkpoints/model_ode_rk4.pth",
            }
            if os.path.exists(legacy[tag]):
                return legacy[tag]
        if tag == "baseline":
            return f"checkpoints/{ds}_gru_d{dim}.pth"
        return f"checkpoints/{ds}_deepm3_{tag}_d{dim}.pth"

    ckpt_map = {
        "baseline": args.baseline_ckpt.strip() or get_ckpt("baseline"),
        "euler": args.euler_ckpt.strip() or get_ckpt("euler"),
        "rk4": args.rk4_ckpt.strip() or get_ckpt("rk4"),
    }

    # Load ODE Euler model
    euler_ckpt = ckpt_map["euler"]
    if os.path.exists(euler_ckpt):
        state = torch.load(euler_ckpt, map_location=device, weights_only=False)
        hidden_dim = infer_hidden_dim(state) or cfg["model"]["hidden_dim"]
        local_cfg = copy.deepcopy(cfg)
        local_cfg["model"]["hidden_dim"] = hidden_dim
        m = DeepM3Model(local_cfg, n_items=dataset.n_items, solver="euler").to(device)
        m.load_state_dict(state, strict=True)
        models["DeepM3 (Euler)"] = (m, "euler")

    # Load ODE RK4 model
    rk4_ckpt = ckpt_map["rk4"]
    if os.path.exists(rk4_ckpt):
        state = torch.load(rk4_ckpt, map_location=device, weights_only=False)
        hidden_dim = infer_hidden_dim(state) or cfg["model"]["hidden_dim"]
        local_cfg = copy.deepcopy(cfg)
        local_cfg["model"]["hidden_dim"] = hidden_dim
        m = DeepM3Model(local_cfg, n_items=dataset.n_items, solver="rk4").to(device)
        m.load_state_dict(state, strict=True)
        models["DeepM3 (RK4)"] = (m, "rk4")

    # Load GRU baseline
    base_ckpt = ckpt_map["baseline"]
    if os.path.exists(base_ckpt):
        state = torch.load(base_ckpt, map_location=device, weights_only=False)
        hidden_dim = infer_hidden_dim(state) or cfg["model"]["hidden_dim"]
        m = GRUBaseline(n_items=dataset.n_items, hidden_dim=hidden_dim).to(device)
        m.load_state_dict(state, strict=True)
        models["Baseline (GRU)"] = (m, "baseline")

    corrupt_modes = [None, "noise", "drop", "both"]
    results = []

    for name, (model, solver) in models.items():
        for mode in corrupt_modes:
            label = mode if mode else "clean"
            print(f"  {name} | {label}... ", end="", flush=True)
            ndcg = eval_model_robust(model, dataset, device, solver,
                                     corrupt_mode=mode, num_neg=args.num_neg,
                                     seed=args.seed, topk=args.topk)
            results.append({"model": name, "condition": label, "ndcg@10": ndcg})
            print(f"{ndcg:.4f}")

    df = pd.DataFrame(results)

    # Compute degradation relative to clean for each model
    pivot = df.pivot(index="model", columns="condition", values="ndcg@10")
    if "clean" in pivot.columns:
        for col in ["noise", "drop", "both"]:
            if col in pivot.columns:
                pivot[f"{col}_drop_pct"] = (
                    (pivot["clean"] - pivot[col]) / (pivot["clean"] + 1e-12) * 100
                )

    os.makedirs(args.output_dir, exist_ok=True)
    out_path = os.path.join(args.output_dir, "time_robustness.csv")
    df.to_csv(out_path, index=False)

    print("\n" + "=" * 60)
    print("Robustness Results:")
    print(pivot.to_string())
    print(f"\nSaved to {out_path}")


if __name__ == "__main__":
    main()
